  ...
  install(Vue, options = {}) {
    ...
    Vue.directive("imageus", {
      async inserted(el, binding) {
        ...
        const mode = el.getAttribute("image-mode");
        const extract = el.getAttribute("image-extract");
        ...
        const srcUrl = queryParams ? `${baseUrl}${src}?${queryParams}` : `${baseUrl}${src}`;

        try {
          const compiled = await compile(`<template><img src="${srcUrl}" /></template>`);
          const { code } = compiled.descriptor.template;
          const Component = Vue.extend({ template: code });
